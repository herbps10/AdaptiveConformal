...
update_faci <- function(object, Y, predictions, X = NULL, training = FALSE) {
  ...
  if(training == TRUE) {
    ...
    for(index in 1:n) {
      ...
      for(k in 1:object$internal$K) {
        losses[k] <- loss_faci(1 - object$alpha, beta, thetas[k])
      ...
      object$internal$losses  <- base::rbind(object$internal$losses, loss_faci(object$alpha, beta, thetabar))
...
predict_faci <- function(object, prediction, X) {
