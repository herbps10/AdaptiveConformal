test_that("AgACI: default initialization", {
  result <- aci(method = "AgACI")

  expect_equal(result$method, "AgACI")
  ...
  expect_equal(length(result$internal$candidate_acis), 8)
})

test_that("AgACI: supplying Y at initialization", {
  result <- aci(c(1, 2, 3), c(0, 1, 2), method = "AgACI")
...
test_that("AgACI: updating with new values", {
  result <- aci(method = "AgACI")
...
test_that("AgACI: X at initialization", {
...
test_that("AgACI: updates with X", {
  X <- matrix(c(1, 0, 0, 1, 1, 1, 1, 0, 1), ncol = 3, nrow = 3)
  result <- aci(X = matrix(ncol = 3, nrow = 0), method = "AgACI")
