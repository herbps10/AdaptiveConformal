test_that("FACI default initialization works", {
  result <- aci(method = "FACI")

  expect_equal(result$method, "FACI")
...
test_that("FACI: works with data at initialization", {
  result <- aci(c(1, 2, 3), c(0, 1, 2), method = "FACI")
...
test_that("FACI: updating with new values", {
  result <- aci(method = "FACI")
...
test_that("FACI: supplying values all at once is equivalent to supplying values one at a time", {
  Y <- rnorm(200, 0, 1)
  predictions <- rnorm(200, 0, 1)
  result1 <- aci(Y = Y, predictions = predictions, method = "FACI", alpha = 0.9)

  result2 <- aci(method = "FACI", alpha = 0.9)
  for(i in 1:200) {
...
test_that("FACI: X at initialization", {
  X <- matrix(c(1, 0, 0, 1, 1, 1, 1, 0, 1), ncol = 3, nrow = 3)
  result <- aci(c(1, 2, 3), c(0, 1, 2), X = X, method = "FACI")

  expect_equal(result$X, X)
  expect_equal(result$metrics$conditional_coverage, matrix(c(0, 2/3, 0.5), nrow = 1))
})

test_that("FACI: updates with X", {
  X <- matrix(c(1, 0, 0, 1, 1, 1, 1, 0, 1), ncol = 3, nrow = 3)
  result <- aci(X = matrix(ncol = 3, nrow = 0), method = "FACI")
