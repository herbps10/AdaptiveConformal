...
## Predictions
For this vignette, we'll train the results first as an initial step and then apply several ACI methods as a second step. 

We start the result training at $t^* = 250$ so that the SARIMA results have enough data to work with.
...
Next, we can try a different ACI method: Fully Adaptive Conformal Inference (FACI). We'll also supply the data directly when we create the ACI object this time, instead of calling the `update` method afterwards.
```{r}
result_faci <- aci(
  Y[t_star:length(Y)],
...
```{r}
plot(result_faci, index = 200:300)
```

If we plot the results from both the AgACI and the FACI methods, we see that they yield quite similar prediction intervals.
```{r}
...
# FACI
lines(result_faci$intervals[200:300, 1], col = "blue")
lines(result_faci$intervals[200:300, 2], col = "blue")

legend("topright", legend = c("AgACI", "FACI"), fill = c("darkred", "blue"))
```
